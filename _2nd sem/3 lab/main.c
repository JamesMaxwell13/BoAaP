...
void changeStruct(books *catalog, int *size)
{
	...
	if(chg != -1)
	{
		...
		printf("|    ID    |         BOOK NAME         |  CR. DATE  |       AUTHOR OF BOOK      |POPULAR.|\n");
		// printf("__________________________________________________________________________________________\n");
		// printf("| %.8ld |", (catalog + chg)->id);
		// printf(" %25s |", (catalog + chg)->name);
		// printf(" %.2d.%.2d.%.4d |", (catalog + chg)->creation.day, (catalog + chg)->creation.month, (catalog + chg)->creation.year);
		// //printf(" %25s |", (catalog + chg)->author);
		// print25((catalog + chg)->author);
		// printf(" %6ld |\n",(catalog + chg)->pop);
...
{
	books *newcat = malloc(20*sizeof(books));
...
int main()
{
	...
	catalog = (books*)malloc(20*sizeof(books));
	char binname[25];
...
\t7 - sort catalog by some fields\n\
\t8 - print N most popular books\n\
\t9 - print all books by cirtain author\n\
\t10 - save catalog to file and exit the program\n\n\
\tyour choose: ");
		command = inputPosInt();
		printf("\n");
		...
		switch(command)
		{
			...
			{	
				...
				s_gets(binname, 25); 
				readStruct(catalog, size, binname);
				break;
			}
			case add_record:
			{
...
After last subsequence enter 0\n");
			...
			case stop:
			{
				...
				printf("Enter the name of bin file: ");
				s_gets(binname, 25); 
				loadStruct(catalog, size, binname);
				*size = 0;
				free(catalog);
				
				books *newcat;
				newcat = (books*)malloc(1*sizeof(books));
				int news = 0, *nsize = &news;
				readStruct(newcat, nsize, binname);
				printStruct(newcat, nsize);
				free(newcat);
				exit(0);
				break;
			}
