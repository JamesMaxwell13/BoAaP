...
int condition(books *catalog, int field[5], int k, int *min, int j)
{
	poke sortf = field[k];
	switch (sortf)
	{
	case id:
		if((catalog + *min)->id > (catalog + j)->id)
			return 1;
		else
			if((catalog + *min)->id == (catalog + j)->id)
				return 0;
	...
	case name:
		if(strCmp((catalog + *min)->name, (catalog + j)->name) > 0)
			return 1;
		else
			if(strCmp((catalog + *min)->name, (catalog + j)->name) == 0)
				return 0;
	...
	case datee:
		return compDate(catalog, *min, j);
		break;

	case author:
		if(strCmp((catalog + *min)->author, (catalog + j)->author) > 0)
			return 1;
		else
			if(strCmp((catalog + *min)->author, (catalog + j)->author) == 0)
				return 0;
	...
	case pop:
		if((catalog + *min)->pop > (catalog + j)->pop)
			return 1;
		else
			if((catalog + *min)->pop == (catalog + j)->pop)
				return 0;
...
void swapStruct(books *catalog, int *min, int i, int j)
{
	books buff;
	*min = j; 
    buff = *(catalog + i);
...
void comparison(books *catalog, int field[5], int k, int *min, int i, int j)
{
	if(field[k] != 0 && k < 5)
	{
		if(condition(catalog, field, k, min, j) == 1)
			swapStruct(catalog, min, i, j);
		else
			if(condition(catalog, field, k, min, j) == 0 && field[k] != 0 && k < 5)
			{	
				k++;
